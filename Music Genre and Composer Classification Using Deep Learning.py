...
specific_artists = ['Bach', 'Beethoven', 'Chopin', 'Mozart'] # These are used for the initial LSTM and CNN analysis
...
class ArtistNotFoundError(Exception):
    def __init__(self, missing_artists):
        ...
        super().__init__(f"The following specific artists are not in the all artists list: {', '.join(missing_artists)}")


# %%
# Get list of all the artist dirs
def get_all_artists(raw_data_extracted):
    all_artists = {name for name in os.listdir(raw_data_extracted) if os.path.isdir(os.path.join(raw_data_extracted, name))}
    return all_artists
...
def initial_start(raw_data_zip, raw_data_extracted, specific_artists):
    ...
    missing_artists = [artist for artist in specific_artists if artist not in all_artists]
...
# %%
def get_midi_lengths_for_artists(raw_data_extracted, specific_artists, graph = True, debug = True):
    ...
    for artist in specific_artists:
        midi_lengths, file_count = get_midi_lengths_for_artist(raw_data_extracted, artist, debug = debug)
...
paths_artist_length_data = get_midi_lengths_for_artists(raw_data_extracted, specific_artists)
...
def process_and_augment_midi_files(raw_data_extracted, specific_artists, output_subdir='augmented_pitch', shifts=[2, -2]):
    ...
    for root, dirs, files in os.walk(raw_data_extracted):
        for file in files:
            if file.endswith('.mid'):
                ...
                if any(artist in os.path.join(root, file) for artist in specific_artists):
                    file_path = os.path.join(root, file)
                    for shift in shifts:
                        augment_midi_pitch_shift(file_path, augmented_pitch_dir, shift=shift)
...
tempo_changes = [tempo for sublist in features_list_df['tempo_changes'] for tempo in sublist]
...
features_list_df['artist_encoded'] = label_encoder.fit_transform(features_list_df['artist'])
...
def plot_training_history(history, figsize=(12, 4)):
    ...
    for i, metric in enumerate(metrics):
        ...
        if metric in history.history:
            ...
            plt.text(0.5, 0.5, f'No {metric} data available', horizontalalignment='center', verticalalignment='center', transform=plt.gca().transAxes)
            plt.title(f'Model {metric}')
            plt.ylabel(metric.capitalize())
            plt.xlabel('Epoch')
    plt.tight_layout()
...
def midi_to_chunks(file_path, chunk_size=150, fs=10):
    ...
    if piano_roll.shape[1] % chunk_size != 0:
        ...
        if last_chunk.shape[1] < chunk_size:
            ...
            last_chunk_padded = np.stack([(last_chunk > 0).astype(int), last_chunk / 127], axis=-1)
...
def process_all_files(df, base_url, fs=10, chunk_size=150):
    ...
    for idx, row in df.iterrows():
        ...
        for i, chunk in enumerate(chunks):
            all_chunks.append([chunk, artist, row['path'], i + 1, chunk.shape[1] < chunk_size])
...
processed_chunk_df = process_all_files(paths_artist_length_data, raw_data_extracted, fs=8, chunk_size=150)
...
def create_model(optimizer='adam', init='glorot_uniform', dropout_rate=0.5):
    model = Sequential([
        Conv2D(32, (3, 3), activation='relu', kernel_initializer=init, input_shape=X.shape[1:]),
        MaxPooling2D((2, 2)),
        ...
        Dense(len(np.unique(processed_chunk_df['Artist'])), activation='softmax', kernel_initializer=init)
    ])
...
# Please do not mind the scrolling. We initially decided to remove the output but decided to keep the results at the last minute as they provide useful information to track back. 
...
paths_artist_length_data_all = get_midi_lengths_for_artists(raw_data_extracted, all_artists, graph= False, debug = False)
...
# %%
processed_chunk_all_df = process_all_files(paths_artist_length_data_all, raw_data_extracted, fs=8, chunk_size=150)
